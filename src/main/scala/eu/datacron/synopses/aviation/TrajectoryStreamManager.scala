/******************************************************************************
  ...
  * File: eu.datacron.synopses.aviation/TrajectoryStreamManager.scala
  * Description: Management of kinematic ADS-B or IFS messages in order to detect critical points upon changes in each object's mobility features (stop, turn, change in altitude, communication gap, etc.).
  *              Input and output streams adhere to specific AVRO attribute schemata (input: location.avsc, output: critical_point.avsc)
  * TODO: Improve noise elimination; verify that all critical points can be detected; fine-tune parametrization.
...
object TrajectoryStreamManager {
  ...
  val url = getClass.getClassLoader.getResource("aviation_config.properties")
  if (url != null) {
  ...
  val kafkaProducer_notifications = new Producer[String, String](configNotifications) //Derived (Kafka) stream for Notifications; essentially, locations with GAP_START annotations emitted with delay
  ...
  def main(args: Array[String]) {
    ...
    System.out.println("clock;feed;node;batch_num_messages;batch_time_ms;total_num_messages;avg_latency_ms")
      ...
      def filter(pos: critical_point): Boolean = {
        ...
        if (config.BBOX.contains(pos.getLongitude, pos.getLatitude)) {         //Only keep raw positions within the specified area of monitoring
          ...
          if (!objStates.contains(pos.getId)) {                   //This is the first location after a communication GAP, ...
            ...
            objStates(pos.getId).initState(pos, config.PARAMS.HISTORY_PERIOD, config.PARAMS.BUFFER_SIZE, config.PARAMS.MAX_ALTITUDE_STOP) //Queue of recent locations is initialized with the current one
          }
          return true
        }
    ...
    val criticalPointStream: DataStream[critical_point] = positionalStream
      ...
      .keyBy(_.getId)        //IMPORTANT: Partition stream by object id
      .countWindow(2, 1)     //Window: use the latest pair of locations reported per object
      .reduce { (prevLoc: critical_point, curLoc: critical_point) => forwardCheckMobilityFeatures(prevLoc, curLoc) }   //FORWARD check: characterize the CURRENT location
      .setParallelism(1)

    //STEP #2 (BACKWARD check): Determine whether the previous location should also be characterized as a turning point or change in altitude (where significant change in heading or change in altitude is observed)
    val turningPointStream: DataStream[critical_point] = criticalPointStream
      ...
      .reduce { (prevLoc: critical_point, curLoc: critical_point) => backwardCheckMobilityFeatures(prevLoc, curLoc) }      //BACKWARD check: characterize the PREVIOUS location
      .setParallelism(1)
    ...
    val jobResult = env.execute("Aviation Trajectory Stream Manager")
  ...
  //Extract constituent values from the ADS-B/IFS message: timestamp, id, lon, lat, alt, on_ground  -- the rest assigned given NULL or zero values (with be calculated during trajectory detection & summarization)
  ...
  def parseLocation(msg : String): (critical_point) = {
    ...
    new critical_point(tokens(0).toLong, tokens(1).toString, tokens(2).toDouble, tokens(3).toDouble, tokens(4).toDouble, null, 0.0F, 0.0F, 0.0F, 0.0F, 0L, tokens(7).toString, System.currentTimeMillis())
  }
  ...
  //BACKWARD check: Detect any significant change in heading or in altitude between two consecutive locations
  ...
  def backwardCheckMobilityFeatures(oldLoc: critical_point, newLoc: critical_point): critical_point = {
    ...
    //IMPORTANT: Check for changes in heading or altitude as long as this object is NOT marked as stopped
    if (!curState.isStopped) {

      //Takeoff: previous locations w.r.t. a location indicating that the aircraft just left the ground; in addition, the rate of climb must be low, in order to avoid noisy flag indications
      //In case this point comes just after a communication gap, isOnGround information may not be reliable
      if ((((newLoc.getOnGround.toString != "-1") && (oldLoc.getOnGround.toString == "-1")) || ((newLoc.getAltitude >= config.PARAMS.MAX_ALTITUDE_STOP) && (oldLoc.getAltitude < config.PARAMS.MAX_ALTITUDE_STOP))) && (oldLoc.getAnnotation.getLanding == false) && (objStates(newLoc.getId).isLanded) && (Math.abs(newLoc.getRateOfClimb) < config.PARAMS.CLIMB_NOISE_THRESHOLD) && (!newLoc.getAnnotation.getGapEnd)) {
        oldLoc.getAnnotation.setTakeoff(true)
        objStates(oldLoc.getId).setLandedStatus(false)
      }

      //Change of heading above threshold w.r.t. previous heading --> CHANGE_IN_HEADING
      //Compare also with the cumulative heading over the recent motion history of this object
      if ((MobilityChecker.angleDifference(newLoc.getHeading, oldLoc.getHeading) > config.PARAMS.ANGLE_THRESHOLD) || (Math.abs(curState.getCumulativeHeading) > config.PARAMS.ANGLE_THRESHOLD)) {
        oldLoc.getAnnotation.setChangeInHeading(true)
      }

      val curMeanRateOfClimb = curState.getMeanRateOfClimb   //Mean rate of climb over the current state (excluding the current location)

      //Change of climb threshold --> CHANGE_IN_ALTITUDE
      //TODO: This rule captures well this phenomenon for IFS data; check if this holds for ADS-B as well
      if ((newLoc.getRateOfClimb >= 0) && ((Math.abs(newLoc.getRateOfClimb) < config.PARAMS.CLIMB_THRESHOLD) && (Math.abs(oldLoc.getRateOfClimb) >= config.PARAMS.CLIMB_THRESHOLD)) ||                         //Ascending
          ((newLoc.getRateOfClimb < 0) && (Math.abs(newLoc.getRateOfClimb - curMeanRateOfClimb) >= config.PARAMS.CLIMB_THRESHOLD) && (Math.abs(oldLoc.getRateOfClimb) < config.PARAMS.CLIMB_THRESHOLD))) {     //Descending
        oldLoc.getAnnotation.setChangeInAltitude(true)
      }


      //If the object changed its heading, remove older positions from its state except for the last two ones
      if (oldLoc.getAnnotation.getChangeInHeading == true) {    //  || (oldLoc.getAnnotation.getChangeInAltitude == true))
        objStates(newLoc.getId).cleanupState()
      }
  ...
  def forwardCheckMobilityFeatures(prevLoc: critical_point, newLoc: critical_point): critical_point = {
    ...
    //Instantaneous rate of climb / descent; return value in feet per second
    newLoc.setRateOfClimb({
      if (newLoc.getTimestamp > oldLoc.getTimestamp)                                                               //timestamps in milliseconds
        1000.0D * MobilityChecker.getAltitudeChange(newLoc, oldLoc) / (newLoc.getTimestamp - oldLoc.getTimestamp)  //signed value: +: climb, -: descent
      else
        0.0D             //Placeholder for NO change
    })
    ...
    //Landing: First location indicating that the aircraft touched the ground; in addition, the rate of climb must be low, in order to avoid noisy flag indications
    //In case this point comes just after a communication gap, isOnGround information may not be reliable
    if ((((newLoc.getOnGround.toString == "-1") && (oldLoc.getOnGround.toString != "-1")) || ((newLoc.getAltitude < config.PARAMS.MAX_ALTITUDE_STOP) && (oldLoc.getAltitude >= config.PARAMS.MAX_ALTITUDE_STOP))) && (!objStates(newLoc.getId).isLanded) && (Math.abs(newLoc.getRateOfClimb) < config.PARAMS.CLIMB_NOISE_THRESHOLD) && (!newLoc.getAnnotation.getGapEnd)) {
      newLoc.getAnnotation.setLanding(true)
      objStates(newLoc.getId).setLandedStatus(true)
    }
    ...
    if (newLoc.getTimeElapsed > 1000.0F * config.PARAMS.GAP_PERIOD)
    {
      ...
      //IMPORTANT: The previously reported location must be marked as GAP_START by pushing a message to another Kafka topic
    ...
    if (newLoc.getAnnotation.getNoise == true) {
      ...
      objStates(newLoc.getId).updateState(newLoc)
    }
    ...
    if ((newLoc.getSpeed < config.PARAMS.NO_SPEED_THRESHOLD) && !curState.isStopped) {
      ...
      //If object appears to be on air with almost zero speed then it should be considered as noise
      if (newLoc.getAltitude > config.PARAMS.MAX_ALTITUDE_STOP) {
        newLoc.getAnnotation.setNoise(true)
        return newLoc                                             //No further processing required
      }

      //Otherwise, this indeed signifies a stop
    ...
    //Either criterion holds: no speed or distance threshold holds --> STOP END
    ...
    if (!curState.isStopped) {
      ...
      if ((newLoc.getSpeed <= config.PARAMS.LOW_SPEED_THRESHOLD) && (oldLoc.getSpeed > config.PARAMS.LOW_SPEED_THRESHOLD) && !curState.inSlowMotion) {
        ...
        if (newLoc.getAltitude > config.PARAMS.MAX_ALTITUDE_STOP)  {  //If slow motion occurs on air then it is considered as noise
          newLoc.getAnnotation.setNoise(true)
          return newLoc               //No further processing required
        }
