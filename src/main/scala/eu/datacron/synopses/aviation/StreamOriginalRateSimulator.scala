/******************************************************************************
  ...
  * File: eu.datacron.synopses.aviation/StreamOriginalRateSimulator.scala
  * Description: Simulates a streaming behaviour by consuming kinematic ADS-B messages from a delimited CSV file, assuming that all messages are in chronological order.
...
object StreamOriginalRateSimulator extends App {
    ...
    //configProperties.load(new FileInputStream("/opt/datacron/config/trajectory_synopses/aviation_config.properties"))    //Fixed file path
    ...
    for (msg <- Source.fromFile(filePath).getLines())
    {
     ...
     //Regulate the amount of messages sent at each second
     if (ts > t0) {
    ...
    //Termination
    producer.close()
    System.out.println(Instant.ofEpochMilli(System.currentTimeMillis()) + " Input stream simulation completed. " + totalMsg + " aviation (ADS-B or IFS) messages consumed in total.")
    ...
    def parseLocation(line : String): (critical_point) = {
     ...
     new critical_point(tokens(0).toLong, tokens(1).toString, tokens(2).toDouble, tokens(3).toDouble, tokens(4).toDouble, null, 0.0F, 0.0F, 0.0F, 0.0F, 0L, tokens(7).toString, 0L)
    }
