/******************************************************************************
  ...
  * File: eu.datacron.synopses.aviation/ObjectState.scala
  * Description: Helper class for maintaining the velocity vector and mobility status for a particular moving object (aircraft) over a recent time interval.
...
//Auxiliary data structure for maintaining state as a queue of recent LOCATIONS per object in order to calculate their respective velocity vectors
...
class ObjectState {
  ...
  var MAX_ALTITUDE_LANDED: Double = 100.0D   //Max altitude (in FEET) for which an aircraft is considered as LANDED
  ...
  var objStatus: Array[Boolean] = Array[Boolean]()          //List of three booleans denoting the current status of an object: 0-bit: STOPPED, 1-bit: SPEED_CHANGED, 2-bit: SLOW_MOTION, 3-bit: LANDED


  //Constructor: initialize state for a given (unique) object identifier
  def initState(curLoc: critical_point, t:Long, n:Int, z_landed: Double) {

    this.id = curLoc.getId
    this.objHistory.enqueue(curLoc)                        //Push this location into the queue
    this.objStatus = Array(false, false, false, false)     //Initialize bitmap
    ...
    this.MAX_ALTITUDE_LANDED = z_landed                    //Max altitude (in FEET) for which an aircraft is considered as LANDED
    initLanded(curLoc)                                     //Initialize status for LANDED
  }
  ...
  def purgeState() {
    ...
    this.objStatus = Array(false, false, false, false)    //...or should this be set to null?
  }
  ...
  //Cleanup the current state (velocity vector) of this object, BUT retain the two latest known location
  def cleanupState() {
  ...
  //Initialize status for LANDED based on the current altitude reported by the first location that is being inserted into the object's state
  def initLanded(curLoc: critical_point) {

    if (curLoc.getAltitude > MAX_ALTITUDE_LANDED)      //Aircraft not reporting very low altitude
      this.setLandedStatus(false)
    else                                               //Otherwise, set the aircraft as landed
      this.setLandedStatus(true)
  }

  //Check if the given object is landed
  def isLanded: Boolean = {

    this.objStatus(3)        //Return value: Fourth item in the List is used for status LANDED
  }


  //Declare this object as landed on ground
  def setLandedStatus(status: Boolean) {

    objStatus(3) = status    //Fourth item in the List is used for status LANDED
  }
  ...
  */
  ...
  /*
    ...
    def getMeanHeading: Double = {
      ...
      Math.atan2(sum_sin, sum_cos)     //Return value between 0 and 359 degrees
    }
  */
...
/*
  ...
  def getMeanHeading: Double = {
    ...
    sum_heading / (1.0D * objHistory.length)    //Return value between 0 and 359 degrees
  }
*/
...
/*
  //Calculate the mean rate of climb/descent for the given object based on its most recent locations recorded in state
  //Take the average of the computed instantaneous rates
  def getMeanRateOfClimb: Double = {

    //Calculate the sum of all values for rate of climb as contained in the queue (already computed once a new location arrives)
    //CAUTION: Exclude the current location, in order not to bias any comparison with the current value
    var sum_rate = (objHistory.foldLeft(0.0D)((acc, v) => acc + v.getRateOfClimb)) - objHistory.last.getRateOfClimb

    sum_rate / (1.0D * objHistory.length)         //Return average value in feet/sec
  }
*/

  //Calculate the mean rate of climb/descent for the given object based on its most recent locations recorded in state
  //ALTERNATIVE RULE: Compute this value from the two extreme positions retained in the state
  def getMeanRateOfClimb: Double = {
    ...
    if (iter.hasNext)
      {
        last = iter.next()     //The location relayed immediately before the last one
      }

    1000.0D * MobilityChecker.getAltitudeChange(last, objHistory.front) / (last.getTimestamp - objHistory.front.getTimestamp)   //Return average value in feet/sec
  }
}