/******************************************************************************
  ...
  * File: eu.datacron.synopses.aviation/MobilityChecker.scala
...
object MobilityChecker {
  ...
  //Time elapsed since the previously reported location
...
*/
  ...
  //Calculate change in altitude between two timestamped locations
  def getAltitudeChange(newLoc: critical_point, oldLoc: critical_point): Double = {
    newLoc.getAltitude - oldLoc.getAltitude      //Return value in feet
  }
...
*/
  ...
  def getBearing(oldLoc: critical_point, newLoc: critical_point): Double = {
    ...
    Math.toDegrees(bearing)       //Return angle value in degrees
  }
  ...
  def angleDifference(heading1: Double, heading2: Double): Double = {
    ...
    if (phi > 180)
      360.0D - phi         //Return angle value in degrees
    else
      phi
  }
  ...
  //Calculate acceleration (sign: +) or deceleration (sign: -) over ground; speed and elapsed time values must have been calculated beforehand
